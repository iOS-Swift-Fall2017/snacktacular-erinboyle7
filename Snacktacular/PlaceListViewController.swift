...
class PlaceListViewController: UIViewController {
    ...
    func loadData() {
        db.collection("places").getDocuments { (querySnapshot, error) in
            ...
            for document in querySnapshot!.documents {
                ...
                let address = docData["placeName"] as! String? ?? ""
    ...
    func saveData(index: Int) {
        ...
        let dataToSave: [String: Any] = ["placeName": places[index].placeName, "address": places[index].address, "postingUserID": places[index].postingUserID, "latitude": latitude, "longitdue": longitude]
        ...
        if places[index].placeDocumentID != "" {
            ...
            //otherwise we don't have a Document ID so we need to create the ref ID and save a new one
            var ref: DocumentReference? = nil //Firestone will create a new ID for us
            ref = db.collection("places").addDocument(data: dataToSave) { (error) in
                if let error = error {
                    print("ERROR: adding document \(error.localizedDescription)")
                } else {
                    print("Document added with reference ID \(String(describing: ref?.documentID))")
...
extension PlaceListViewController: FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        if let user = user {
            print("*** Successully logeed in with user = \(user.email!)")
        }
    }
